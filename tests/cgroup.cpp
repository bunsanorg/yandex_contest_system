...
BOOST_AUTO_TEST_CASE(attached) {
  const yac::SingleControlGroupPointer acg = yac::SingleControlGroup::attach(
      processHierarchy.hierarchy.id, cg->controlGroup());
  BOOST_CHECK_EQUAL(acg->tasks().size(), 0);
  BOOST_CHECK_EQUAL(acg->controlGroup(), cg->controlGroup());
  BOOST_CHECK_THROW(acg->attachChild("123"),
                    yac::SingleControlGroupNotExistsError);
}

BOOST_AUTO_TEST_CASE(created) {
  const yac::SingleControlGroupPointer thisCG2 =
      yac::SingleControlGroup::attach(processHierarchy.hierarchy.id,
                                      thisCG->controlGroup());
  BOOST_CHECK_THROW(thisCG2->createChild(cg->controlGroup().filename()),
                    yac::SingleControlGroupExistsError);
}
...
BOOST_AUTO_TEST_CASE(attachChild) {
  const yac::SingleControlGroupPointer acg =
      thisCG->attachChild(cg->controlGroup().filename());
...
BOOST_AUTO_TEST_CASE(forSelf) { BOOST_TEST_MESSAGE(*thisCG); }
...
BOOST_AUTO_TEST_CASE(fieldPath) {
  BOOST_CHECK(
      boost::filesystem::is_regular_file(cg->fieldPath("freezer.state")));
  BOOST_CHECK_THROW(thisCG->fieldPath("/"),
                    yac::ControlGroupInvalidFieldNameError);
  BOOST_CHECK_THROW(thisCG->fieldPath("unknown/path"),
                    yac::ControlGroupInvalidFieldNameError);
  BOOST_CHECK_THROW(thisCG->fieldPath("unknown.path"),
                    yac::ControlGroupFieldDoesNotExistError);
  BOOST_CHECK_THROW(thisCG->fieldPath(cgName.string()),
...
BOOST_AUTO_TEST_CASE(container) {
  ...
  const yac::SingleControlGroupPointer cgroup2 =
      yac::SingleControlGroup::forSelf(freezerId);
  BOOST_CHECK_EQUAL(cgroup2->hierarchyId(), freezerId);
  BOOST_CHECK_THROW(thisCG->add(cgroup),
                    yac::MultipleControlGroupHierarchyConflictError);
  thisCG->remove(freezerId);
  BOOST_CHECK_THROW(thisCG->remove(freezerId),
...
BOOST_AUTO_TEST_CASE(fields) {
  ...
  BOOST_CHECK_THROW(cg->cloneChildren(),
                    yac::MultipleControlGroupFieldValueConflictError);
  const std::vector<std::size_t> empty;
  BOOST_CHECK_THROW(yac::MultipleControlGroup::root(empty.begin(), empty.end())
                        ->cloneChildren(),
