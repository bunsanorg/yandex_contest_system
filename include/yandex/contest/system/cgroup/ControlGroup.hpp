...
struct ControlGroupFieldError : virtual ControlGroupError {
  using fieldName = boost::error_info<struct fieldNameTag, std::string>;
  using fieldPath =
      boost::error_info<struct fieldPathTag, boost::filesystem::path>;
};
...
class ControlGroup : public IntrusivePointeeBase {
 public:
  using Tasks = std::unordered_set<pid_t>;
  using Reader = std::function<void(std::istream &)>;
 ...
 public:
  ...
  virtual void setNotifyOnRelease(bool notifyOnRelease = true) = 0;
 ...
 protected:
  ...
  virtual ControlGroupPointer createChild__(
      const boost::filesystem::path &childControlGroup, mode_t mode) = 0;
