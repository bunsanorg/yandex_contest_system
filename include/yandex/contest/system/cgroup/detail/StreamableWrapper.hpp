...
#define YANDEX_CONTEST_SYSTEM_CGROUP_DETAIL_STREAMABLE_WRAPPER(PREFIX, STREAM, OPERATOR) \
    ...
    public: \
        ...
        PREFIX##StreamableWrapper &operator=(const PREFIX##StreamableWrapper &)=default; \
        PREFIX##StreamableWrapper &operator=(PREFIX##StreamableWrapper &&)=default; \
 \
        template <typename T> \
 ...
 \
            void dispatch(STREAM &stream) override \
            { \
                stream OPERATOR *object_; \
            } \
 \
        private: \
            T *object_; \
