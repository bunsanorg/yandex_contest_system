...
ControlGroupPointer ControlGroup::parent() { return parent__(); }
...
void ControlGroup::attachSelf() { attachTask(unistd::getpid()); }
...
void ControlGroup::readField(const std::string &fieldName,
  ...
  readFieldByReader(fieldName, [&data](std::istream &in) { in >> data; });
}
...
std::string ControlGroup::readFieldAll(const std::string &fieldName) {
  std::string data;
  readFieldByReader(fieldName, [&data](std::istream &in) {
