  ...
  for (const unistd::MountEntry &entry : fstab) {
    BUNSAN_EXCEPTIONS_WRAP_BEGIN() {
      if (entry.type == "cgroup") {
        ...
        boost::algorithm::split(opts, entry.opts,
...
SystemInfo::const_iterator SystemInfo::begin() const { return cbegin(); }

SystemInfo::const_iterator SystemInfo::cbegin() const {
  return const_iterator(id2hierarchy_.cbegin());
}

SystemInfo::const_iterator SystemInfo::end() const { return cend(); }
