            ...
            while (std::getline(fin, line))
            {
                ...
                BUNSAN_EXCEPTIONS_WRAP_BEGIN()
                {
                    ...
                    if (pos1 == std::string::npos)
                        BOOST_THROW_EXCEPTION(ProcPidCgroupNotEnoughFieldsError());
                    const std::size_t pos2 = line.find(':', pos1 + 1);
                    if (pos2 == std::string::npos)
                        BOOST_THROW_EXCEPTION(ProcPidCgroupNotEnoughFieldsError());
                    BOOST_ASSERT(pos1 < pos2);
                    // Since last field may contain ':', we do not check exact number of ':'-separated fields.
                    entry.hierarchyId = boost::lexical_cast<std::size_t>(line.substr(0, pos1));
                    const std::string subsystemsLine = line.substr(pos1 + 1, pos2 - pos1 - 1);
                    boost::algorithm::split(entry.subsystems, subsystemsLine, boost::algorithm::is_any_of(","));
