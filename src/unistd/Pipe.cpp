...
enum Pipe::End : unsigned { READ_END, WRITE_END };

Pipe::Pipe() {
  int fildes_[2];
  if (::pipe(fildes_) < 0) BOOST_THROW_EXCEPTION(SystemError("pipe"));
  for (int i = 0; i < 2; ++i) fd_[i].assign(fildes_[i]);
}
...
Descriptor Pipe::releaseReadEnd() noexcept { return std::move(fd_[READ_END]); }
...
void Pipe::closeReadEnd() { fd_[READ_END].close(); }

void Pipe::closeWriteEnd(std::error_code &ec) noexcept {
  fd_[WRITE_END].close(ec);
}

void Pipe::closeWriteEnd() { fd_[WRITE_END].close(); }
