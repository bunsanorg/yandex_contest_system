...
DynamicLibrary::DynamicLibrary(const boost::filesystem::path &filename,
                               const int flags)
    : handle_(dlopen(filename.c_str(), flags | RTLD_NOLOAD)) {
  const boost::filesystem::path fname =
      (filename.filename() == filename)
          ? str(boost::format("lib%1%.so") % filename.string())
          : filename;
  if (handle_)
...
void DynamicLibrary::closeNoExcept() noexcept {
  try {
    ...
    STREAM_ERROR << "Error while closing DynamicLibrary (ignoring): "
                 << e.what();
  }
}
...
DynamicLibrary::operator bool() const noexcept { return handle_; }
...
DLInfo dladdr(void *const addr) {
  ...
  if (!dladdr(addr, &info)) {
    const char *const err = dlerror();
    BOOST_ASSERT(err);
    BOOST_THROW_EXCEPTION(DynamicLoaderError()
                          << DynamicLoaderError::dlerror(err));
  }
  DLInfo ret;
  ret.fname = info.dli_fname;
  ret.fbase = info.dli_fbase;
  if (info.dli_sname) ret.sname = typeinfo::demangle(info.dli_sname);
  if (info.dli_saddr) {
    ret.saddr = info.dli_saddr;
    ret.offset =
        static_cast<char *>(addr) - static_cast<char *>(info.dli_saddr);
  }
  return ret;
}
}  // namespace detail
