...
Exec::Exec(const boost::filesystem::path &executable,
    ...
    : executable_(executable),
      argvWrapper_(
          arguments.empty() ? ProcessArguments{executable.string()} : arguments,
          CharStarStar::stringToVectorChar),
      envpWrapper_(environment, stringPairToVectorChar) {}

const char *Exec::executable() const { return executable_.c_str(); }

char **Exec::argv() { return argvWrapper_.get(); }

char **Exec::envp() { return envpWrapper_.get(); }
