...
Descriptor::~Descriptor() { closeNoExcept(); }
...
void Descriptor::close() {
  std::error_code ec;
  close(ec);
  if (ec) BOOST_THROW_EXCEPTION(SystemError(ec, __func__));
}
...
void Descriptor::close(std::error_code &ec) noexcept {
  ...
  if (fd_) {
    ...
    if (ret < 0) ec.assign(errno, std::system_category());
    fd_.reset();
  }
}
...
Descriptor::operator bool() const noexcept { return static_cast<bool>(fd_); }
